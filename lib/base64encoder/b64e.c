...
static int readstring_to_mem(const unsigned char* str, unsigned char** buffer, size_t* size_out){
  size_t s = 0;
  char c;
  while(1){
    c = str[s];
...
static int readfile_to_mem(const unsigned char* filename, unsigned char** buffer,size_t* size_out){
  FILE* f;
  long size;
...
static int writefile_from_mem(const unsigned char* filename, const unsigned char* buffer,size_t size){
  FILE* f;
  size_t n;
  f = fopen(filename,"wb");
